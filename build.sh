...
system_image=kaytime-dex-$(printf "$OS_VERSION")-amd64.iso
root_fs=kaytime-dex-rootfs-$(printf "$OS_VERSION").tar
root_fs_latest=kaytime-dex-rootfs-latest.tar
...
runch dex.sh $GIT_CURRENT_BRANCH \
...
cp "$root_fs.xz" "$root_fs_latest.xz"
